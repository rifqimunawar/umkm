...
@section('module-content')
  ...
  <div class="panel panel-inverse">
    ...
    <div class="card-body m-4">
      <div>
        <form action="{{ route('m2block.store') }}" method="post" class="row" enctype="multipart/form-data">
          @csrf

          <div class="form-group mb-2">
            <label for="nomor_m2block">Nomor</label>
            <input type="number" autofocus class="form-control" name="nomor_m2block" id="nomor_m2block"
              placeholder="Nomor" required value="">
          </div>

          <div class="card-action gap-2 d-flex justify-content-start mt-4">
            <button class="btn btn-success btn-sm" type="submit">Simpan</button>
        </form>
        <p>Nomor selanjutnya: {{ $next }}</p>
        <p>Nomor selanjutnya: {{ $next1 }}</p>
        <p>Nomor selanjutnya: {{ $next2 }}</p>
      </div>
    </div>
    <div
      style="display: flex; justify-content: space-between; align-items: center; margin-right: 15px;margin-top:10px;margin-left:15px">
      {{-- <a href="{{ route('m2block.create') }}" class="btn btn-primary btn-sm">Tambah &ensp;<i class="fa fa-plus-square"
          aria-hidden="true" style="font-size: 12px"></i></a> --}}
      <div style="display: flex; gap: 10px;">
        {{-- <a href="{{ route('m2block.export') }}" class="btn btn-warning btn-sm">
        ...
        {{-- <a href="{{ route('m2block.pdf') }}" class="btn btn-warning btn-sm">
    ...
    <div class="panel-body">
      <table id="data-table-default" width="100%" class="table table-striped table-bordered align-middle text-nowrap">
        <thead>
          <tr>
            <th width="1%"></th>
            <th class="text-nowrap">Nomor Prediksi</th>
