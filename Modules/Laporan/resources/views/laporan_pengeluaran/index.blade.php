  ...
  <div class="panel panel-inverse">
    ...
    {{-- <div
      ...
      <div style="display: flex; align-items: center; gap: 10px;">
        <a href="{{ route('lap_pengeluaran.export') }}" class="btn btn-warning btn-sm">
      ...
      <div style="display: flex; gap: 10px;">
        <a href="javascript:void(0)" class="btn btn-warning btn-sm"
          onclick="printPage('{{ route('lap_pengeluaran.print') }}')">
          <i class="fa fa-print" aria-hidden="true"></i> Print
        </a>
    </a>
    <a href="{{ route('lap_pengeluaran.pdf') }}" class="btn btn-warning btn-sm">
      <i class="fa fa-file-pdf" style="font-size: 12px"></i> Export PDF
    </a>
  </div>
  </div> --}}

    <form method="GET" action="{{ route('lap_pengeluaran.index') }}"
      style="display: flex; justify-content: space-between; align-items: center; margin-right: 15px;margin-top:10px;margin-left:15px">
      <div style="display: flex; align-items: center; gap: 10px;">
        <a href="{{ route('lap_pengeluaran.export', ['start_date' => request('start_date', date('Y-m-d')), 'end_date' => request('end_date', date('Y-m-d'))]) }}"
      ...
      <div style="display: flex; gap: 10px;">
        {{-- <a href="{{ route('lap_pengeluaran.print', ['start_date' => request('start_date', date('Y-m-d')), 'end_date' => request('end_date', date('Y-m-d'))]) }}" class="btn btn-danger btn-sm">
          <i class="fa fa-print" style="font-size: 12px"></i> Print Demo --}}
        <a href="javascript:void(0)" class="btn btn-warning btn-sm"
          onclick="printPage('{{ route('lap_pengeluaran.print', ['start_date' => request('start_date', date('Y-m-d')), 'end_date' => request('end_date', date('Y-m-d'))]) }}')">
          <i class="fa fa-print" aria-hidden="true"></i> Print
        </a>
        <a href="{{ route('lap_pengeluaran.pdf', ['start_date' => request('start_date', date('Y-m-d')), 'end_date' => request('end_date', date('Y-m-d'))]) }}"
          class="btn btn-warning btn-sm">
          <i class="fa fa-file-pdf" style="font-size: 12px"></i> Export PDF
        </a>
      </div>
        ...
        <tbody>
          ...
          @foreach ($data as $item)
            ...
            <tr class="odd gradeX">
              ...
              <td>{{ Fungsi::rupiah($item->nominal ?? 0) }}</td>
