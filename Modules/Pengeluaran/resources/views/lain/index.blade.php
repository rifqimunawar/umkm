...
@section('module-content')
  ...
  <div class="panel panel-inverse">
    ...
    <div
      ...
      <a href="{{ route('pengeluaran_lain.create') }}" class="btn btn-primary btn-sm">Tambah &ensp;<i
          class="fa fa-plus-square" aria-hidden="true" style="font-size: 12px"></i></a>
      <div style="display: flex; gap: 10px;">
        {{-- <a href="{{ route('pengeluaran_lain.export') }}" class="btn btn-warning btn-sm">
        ...
        {{-- <a href="{{ route('pengeluaran_lain.pdf') }}" class="btn btn-warning btn-sm">
      ...
      <table id="data-table-default" width="100%" class="table table-striped table-bordered align-middle text-nowrap">
        <thead>
          <tr>
            ...
            <th class="text-nowrap">Descripsi</th>
        ...
        <tbody>
          @foreach ($data as $item)
            <tr class="odd gradeX">
              ...
              <td>{{ $item->nama_pengeluaran_lain }}</td>
              <td>{{ Fungsi::rupiah($item->nominal_pengeluaran_lain) }}</td>
              <td>{{ $item->desc ?? '-' }}</td>
              <td class="d-flex justify-content-center align-items-center text-center">
                <a href="{{ route('pengeluaran_lain.edit', $item->id) }}">
                  <i class="fa fa-pencil-square" style="font-size: 14px; margin-right:5px"></i>
                </a>
                <a href="{{ route('pengeluaran_lain.destroy', $item->id) }}" data-confirm-delete="true">
