...
@section('module-content')
  ...
  <div class="panel panel-inverse">
    ...
    <div
      ...
      <a href="{{ route('produk.create') }}" class="btn btn-primary btn-sm">Tambah &ensp;<i class="fa fa-plus-square"
          aria-hidden="true" style="font-size: 12px"></i></a>
      <div style="display: flex; gap: 10px;">
        {{-- <a href="{{ route('produk.export') }}" class="btn btn-warning btn-sm">
        ...
        {{-- <a href="{{ route('produk.pdf') }}" class="btn btn-warning btn-sm">
      ...
      <table id="data-table-default" width="100%" class="table table-striped table-bordered align-middle text-nowrap">
        <thead>
          <tr>
            ...
            <th class="text-nowrap">Harga Beli Satuan</th>
            <th class="text-nowrap">Harga Jual</th>
            <th class="text-nowrap">Harga Jual Satuan</th>
            <th class="text-nowrap">Margin Keuntungan</th>
        ...
        <tbody>
          @foreach ($data as $item)
            <tr class="odd gradeX">
              ...
              <td>{{ Fungsi::rupiah($item->harga_beli ?? '0') }}</td>
              <td>{{ Fungsi::rupiah($item->harga_beli_satuan ?? '0') }}</td>
              <td>{{ Fungsi::rupiah($item->harga_jual ?? '0') }}</td>
              <td>{{ Fungsi::rupiah($item->harga_jual_satuan ?? '0') }}</td>
              <td>{{ Fungsi::rupiah($item->margin_nominal ?? '0') }} / {{ $item->margin_presentase }}%</td>
              ...
              <td class="d-flex justify-content-center align-items-center text-center">
                ...
                <a href="{{ route('produk.destroy', $item->id) }}" data-confirm-delete="true">
